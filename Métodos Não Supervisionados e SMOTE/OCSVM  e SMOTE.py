...
from sklearn.svm import OneClassSVM  # O modelo de detecção de anomalias (OCSVM).
...
OCSVM_THRESHOLD = 0.0  # Limiar de decisão padrão.
...
def evaluate_anomaly_model(model, X_test, y_test, threshold):
    """Avalia o OCSVM baseado na predição."""
    # OCSVM retorna 1 para normal e -1 para anomalia/fraude.
...
OCSVM_PARAMS = {
    'kernel': 'rbf',
    # Ao treinar o OCSVM em dados balanceados (50/50), definimos nu=0.5
    # para que ele tente encapsular cerca de 50% dos dados como a classe "normal" (0)
    # e 50% como a classe "outlier" (1), agindo como um classificador binário.
    'nu': 0.5,
    'gamma': 'auto',
}

print("\n--- OCSVM (SMOTE) - Configuração ---")
...
model_ocsvm_smote.fit(X_res, y_res)

print("Treinamento do OCSVM (SMOTE) concluído.")
...
print_results("One-Class SVM TREINADO com SMOTE - Resultados no Teste", results_ocsvm_smote)
print("=" * 60)