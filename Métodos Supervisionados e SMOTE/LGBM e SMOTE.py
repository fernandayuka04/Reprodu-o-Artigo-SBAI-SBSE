...
from imblearn.over_sampling import SMOTE  # Técnica de Sobreamostragem de Minoria Sintética (para balanceamento).
...
# Os parâmetros do LGBM são configurados para otimizar o desempenho e evitar overfitting.
LGBM_PARAMS = {
...
model_lgbm_smote = LGBMClassifier(**LGBM_PARAMS)
# O treinamento é feito no conjunto balanceado SMOTE.
model_lgbm_smote.fit(X_res, y_res)

print("Treinamento do LGBM (SMOTE) concluído.")

# 4.5 Avaliação
# A avaliação é feita no conjunto de teste original (X_test_scaled)
results_lgbm_smote = evaluate_lgbm_model(model_lgbm_smote, X_test_scaled, y_test)
print_results("LGBM Classifier TREINADO com SMOTE - Resultados no Teste", results_lgbm_smote)
print("=" * 60)