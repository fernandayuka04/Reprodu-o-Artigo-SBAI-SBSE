...
X_res, y_res = sm.fit_resample(X_train_scaled, y_train_full)  # Gera amostras sintéticas.
...
print("\n--- Random Forest (SMOTE) - Configuração ---")
...
model_rf_smote = RandomForestClassifier(**RF_PARAMS)
model_rf_smote.fit(X_res, y_res)

print("Treinamento do Random Forest (SMOTE) concluído.")

# 4.5 Avaliação
results_rf_smote = evaluate_classifier_model(model_rf_smote, X_test_scaled, y_test)
print_results("Random Forest Classifier TREINADO com SMOTE - Resultados no Teste", results_rf_smote)
print("=" * 60)